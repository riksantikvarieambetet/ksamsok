(function(e) {
	...
	var h = "";
	var l = [ "January", "February", "March", "April", "May", "June", "July",
			"August", "September", "October", "November", "December" ];
	for (var u = 0; u < l.length; u++) {
		h += "<option value='" + (u + 1) + "'>" + l[u] + "</option>\n"
	}
	var s = "";
	var g = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday",
			"Saturday" ];
	...
	function z(A, j) {
		...
		for (var B = 0; B < C.length; B++) {
			...
			e.error("cron: invalid value found (col " + (B + 1) + ") in "
					+ o.initial);
			return undefined
		}
		for ( var H in w) {
		...
		if (a(i.customValues)) {
			for (key in i.customValues) {
				if (w.hasOwnProperty(key)) {
					e.error("cron: reserved keyword '" + key
	...
	var x = {
		init : function(i) {
			...
			e.extend(B, {
				...
				timeMinuteOpts : e.extend({}, n.timeMinuteOpts, j,
						G.timeMinuteOpts)
			});
			if (f(this, B)) {
				return this
			}
			var C = [], A = "", D = B.customValues;
			if (a(D)) {
				for ( var F in D) {
			...
			C.period = e(
					"<span class='cron-period'>Every <select name='cron-period'>"
							+ A + r + "</select> </span>").appendTo(this).data(
					"root", this);
			...
			C.dom = e(
					"<span class='cron-block cron-block-dom'> on the <select name='cron-dom'>"
							+ v + "</select> </span>").appendTo(this).data(
					"root", this);
			...
			C.month = e(
					"<span class='cron-block cron-block-month'> of <select name='cron-month'>"
							+ h + "</select> </span>").appendTo(this).data(
					"root", this);
			...
			C.mins = e(
					"<span class='cron-block cron-block-mins'> at <select name='cron-mins'>"
							+ y + "</select> minutes past the hour </span>")
					.appendTo(this).data("root", this);
			...
			C.dow = e(
					"<span class='cron-block cron-block-dow'> on <select name='cron-dow'>"
							+ s + "</select> </span>").appendTo(this).data(
					"root", this);
			...
			C.time = e(
					"<span class='cron-block cron-block-time'> at <select name='cron-time-hour' class='cron-time-hour'>"
							...
							+ y + " </span>").appendTo(this).data("root", this);
			...
			C.controls = e(
					...
					.appendTo(this).data("root", this).find(
							"span.cron-button-save").bind("click.cron",
							m.saveClicked).data("root", this).end();
			this.find("select")
					.bind("change.cron-callback", m.somethingChanged);
	...
	var m = {
		periodChanged : function() {
			...
			var E = A.data("block"), C = A.data("options");
			var D = e(this).val();
